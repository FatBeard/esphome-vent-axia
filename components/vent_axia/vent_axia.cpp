...
void VentAxiaHub::setup() {
  ...
  // Display never touches text_sensor::TextSensor -- the hub does all publishing.
  // The callback only fires for lines that actually changed, so a change to
  // line1 alone never re-publishes line2.
  this->display_.set_on_change([this](bool line1_changed, bool line2_changed) {
    if (line1_changed) {
      // Presentation lane: becomes an HA text sensor's state, which ESPHome's
      // native API encodes as protobuf UTF-8 -- never the raw lane.
      this->publish_text_(TextKey::DISPLAY_LINE_1, this->display_.text_line1());
      // status_message is a trimmed sibling of display_line_1 for the status
      // loop specifically. Gated on screen_kind() so a menu screen's line1
      // (e.g. "Set Clock") never overwrites it -- the entity holds its last
      // status-loop value while a sequence is elsewhere. parser::trim strips
      // only ASCII 0x20, which stays correct against the UTF-8 presentation lane.
    ...
    // Diagnostic decode is entirely passive: whatever page the display happens
    // to be showing -- browsed by hand, or scrolled by a fetch sequence -- gets
    // fed through here. Gated on either line changing, not fired unconditionally
    // every ~300ms frame, so sitting still on one page doesn't spam identical
    // publishes; `line1_changed` alone would miss a page whose content changes
    // without its line1 changing, so both are checked. Both reads below are the
    // RAW lane -- diagnostic_page() is a byte-offset decoder, and
    // publish_diagnostic_page_() does the one necessary transcode for its two
    // presentation uses.
  ...
  // Keypad is portable core (no ESPHome headers), so it reaches the UART and
  // the logger only through these two injected sinks. read_only_ is forwarded
  // here rather than duplicated: one flag, one meaning, checked only in
  // Keypad::maybe_transmit_.
  ...
  // Runner is portable core too, same LogSink shape as Keypad above --
  ...
  // frame_logger_ is portable core too. ESP_LOGD, not ESP_LOGI/W/E like
  // keypad_/runner_ above: a raw frame arrives ~3.3 times a second, and wiring
  ...
  // read_settings_ (the button's own instance) and write_setting_ (shared by
  // write_switch()/write_number(), whose own read-back step is a SEPARATE
  // ReadSettings member) both publish through the exact same two sinks, so a
  // value read by either path reaches the same entities the same way.
  ...
  // sync_clock_ gets wall-clock time through this sink rather than touching
  // time::RealTimeClock itself -- sequence.h/seq_sync_clock.cpp are portable
  // core. Must compile with USE_TIME undefined (the ESP32-IDF example declares
  // no `time:` platform at all): the #else branch below is what makes that
  // true, returning "unavailable" rather than failing to build.
  this->sync_clock_.set_time_source([this](int &dow_display, int &hour, int &minute) -> bool {
    ...
    // Not const: ESPTime::strftime() is a non-const member. This callback
    // never calls it, but `now` is left non-const so a future edit that adds
    // one doesn't have to remember the rule.
    auto now = this->time_->now();
    if (!now.is_valid()) {
      // Not synced yet -- counts as "unavailable" the same as no time source
      // at all, because syncing the unit against an unsynced clock would
      // write a WRONG time.
      return false;
    }
  ...
  // CHECK_CURRENT needs the alternation-aware, sticky purging() to answer "is
  // the unit currently purging" reliably -- the exact same tracker every other
  // status-derived entity reads from, not a second, separate purge decode.
  // status_ outlives set_airflow_mode_ (both are hub members), so this
  // pointer stays valid for the hub's whole lifetime.
  this->set_airflow_mode_.set_status(&this->status_);

  // reset_filter_ is the one irreversible operation. Its OWN dedicated
  // FetchDiagnostics child (NOT fetch_diagnostics_ above) needs the same
  // on_success stamp every other diagnostic scrape gets: a genuine full scan
  // happens inside its own chained scan exactly as for the button/schedule
  // path, so the timestamp should reflect it the same way.
  this->reset_filter_.set_on_diagnostics_success([this] { this->stamp_diagnostics_updated_(); });
  // Least invasive route to page 23's reading: reuses the SAME "last
  // published" cache publish_sensor_ already keeps for its own dedup, rather
  // than inventing a second, parallel cache just for this one check.
...
void VentAxiaHub::loop() {
  ...
  while (this->available() != 0 && this->read_byte(&byte)) {
    if (this->framer_.feed(byte, &frame)) {
      ...
      // Reads frame.line1/line2 directly rather than display_'s raw lane: this
      // call runs BEFORE display_.update() below, so display_'s raw lane still
      // holds the PREVIOUS frame at this point, not the one being logged.
      // Placed first to read that way, not because update() mutates the
      // frame: it takes const refs, so `frame` is untouched either side.
      ...
      // needs a regular heartbeat to notice time passing, and a repeated
      // identical frame is exactly the case where "nothing changed, still
      // true" has to be confirmed rather than silently skipped. Raw lane:
      // every status:: decoder is a fixed-offset byte reader, never a
  ...
  // Drives every timing decision in Keypad. Checked every tick regardless of
  // whether a frame arrived, same reasoning as link_up above: a held key needs
  // retransmitting on the clock, not on an event tied to the MVHR's own
  // ~300ms display refresh.
  this->keypad_.loop(now);

  // Runner sits alongside Keypad, not on top of it: it directs the keypad
  // (via HoldUntil/Tap et al.) rather than owning its timing loop, so both get
  // pumped independently every tick with the same now_ms. Fed link_up_ fresh
  // each tick too -- Runner::request() refuses to start anything while the
  // link is down.
  this->runner_.set_link_up(this->link_up_);
  this->runner_.loop(now);

  // A sequence can be between keypresses (settling, waiting on a predicate)
  // and still be busy as far as a dashboard is concerned.
...
void VentAxiaHub::dump_config() {
  ...
  // The timing constants in force -- every one paid for with debugging on the
  // live unit -- plus the count of presses that emitted fewer than 2 frames.
  // Reported here so a soak test can catch a creeping problem without
...
void VentAxiaHub::tap_key(protocol::KeyMask mask, uint32_t duration_ms) {
  // Runner::tap() is the interlock choke point -- its return value is
  // intentionally ignored here: a refusal is already logged there (runner_'s
  // LogSink, wired in setup()), and there is nothing more for a
  // fire-and-forget caller like a button press to do about it.
...
void VentAxiaHub::write_switch(SwitchKey key, bool state) {
  // The mapping itself lives in the portable core (setting_for()) so a
  // forgotten key is caught by the host suite's -Werror rather than only
  // warned about in a firmware build log.
...
void VentAxiaHub::publish_sensor_(SensorKey key, std::optional<int> value) {
  ...
  if (!value.has_value()) {
    // If nothing was ever published, the tracker simply doesn't know yet:
    // publish nothing, so the entity stays unknown rather than becoming a
    // guess. But if a value *was* published and has now gone away, the last
    // one is stale and misleading -- boost_time_remaining is the case that
    // matters: when a timed boost ends, the countdown disappears from line2
    // and HA would otherwise go on showing e.g. "30 min" indefinitely. There
    // is no way to un-publish, so publish NaN, which HA renders as unknown.
...
void VentAxiaHub::publish_airflow_mode_() {
  ...
  // deliberately passes through boost states nobody chose, and letting the
  // tracker's latch absorb that transient noise would corrupt the evidence it
  ...
  // sequences sharing a name() could otherwise defeat a string check.
...
void VentAxiaHub::publish_diagnostic_page_(uint8_t page, const std::string &line2) {
  ...
  // transcoded exactly ONCE, for the two presentation uses immediately below,
  // so the cost is one to_utf8() call per page-change rather than two.
  // diagnostics::decode_page() below keeps reading the RAW bytes: every
  // diagnostic page ever observed on this unit is pure ASCII, so today the
  // two representations are byte-identical, but the decoder must stay
  // correct if a page ever isn't.
  const std::string text_line2 = to_utf8(line2);

  // Raw escape hatch and trigger fire for every page seen, decoded by the
  // table or not -- lets a page nobody has taught diagnostics.cpp to
  // understand yet (or a nonexistent page 28, or anything else) stay visible
  // from YAML without a component change. Both are presentation uses (a text
  // sensor state and a YAML automation trigger's std::string argument), hence
  // text_line2.
...
void VentAxiaHub::reconcile_filter_change_due_(bool due_from_page23) {
  ...
  // (the scheduled fetch) or whenever a human browses the menu by hand. Live
  // wins by recency: the diagnostic-derived value is only published when the
  // live tracker has no opinion yet -- e.g. straight after boot, before the
  // first status-screen frame -- otherwise this purely cross-checks and logs
  // a disagreement, because a disagreement means one of the two decodes is
...
void VentAxiaHub::publish_link_up_(uint32_t now_ms) {
  ...
  // once LINK_TIMEOUT_MS has passed without a new one -- deliberately not
  // inferred from line2 having stopped republishing (a settled screen looks
  ...
  // refuses to start anything while the link is down, so it needs the same
  // answer this publishes, not a second computation of it.
  this->link_up_ = up;
...
void VentAxiaHub::stamp_diagnostics_updated_() {
  ...
  auto now = this->time_->now();
  if (!now.is_valid()) {
    // Not logged as a warning: routine for the first run or two after boot,
    // before HA's `time: homeassistant` platform has completed its first
    // sync -- a scheduled fetch on a long-running device essentially never
    // sees this.
    return;
  }
