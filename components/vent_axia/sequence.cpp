...
Poll Sequence::await(Sequence &child, uint8_t on_ok) {
  if (!this->runner_->push_child_(child, on_ok)) {
    ...
    // this reports the failure synchronously as if the child had failed on
    // its own first poll(): the caller returns FAILED, which Runner::loop()
...
Poll Sequence::tap_then_(protocol::KeyMask mask, uint8_t next_step) {
  // this->runner_->tap_duration_ms(), NOT a hardcoded duration: every
  // sequence tap goes through here (or one of the handful of batch-tap sites
  // this helper doesn't fit) so raising tap_duration reaches all of them, not
  // just the four manual key buttons.
...
void Runner::loop(uint32_t now_ms) {
  ...
  // The per-root backstop, measured against the ROOT's own timeout_ms()
  // regardless of which frame is currently on top of the stack: individual
  // steps are expected to carry their own, much shorter timeouts (HoldUntil
  // takes one explicitly) -- this only catches the case where something
  // above failed to, or several short steps together ran long enough to add
  ...
  // Mid-run link loss. request() only refuses to START a run while the link
  // is down -- nothing rechecks link_up_ once depth_ > 0, so a drop partway
  // through was previously only ever caught by the ROOT timeout running all
  // the way out (up to 450s for SyncClock, 480s for WriteSetting). By then
  // the display has stopped updating too, so every predicate that reads it
  // has stalled -- AdjustField::CHECK in particular: its guard_count_ only
  // increments after a SUCCESSFUL parse (step 3 below), so a frozen display
  // never advances it either, and the run just keeps queueing taps at a unit
  // that isn't listening until the root timeout finally does the job this
  // check now does much sooner. No extra debounce needed: the hub computes
  // link_up as `have_frame_ && (now - last_frame_at_ms_) < LINK_TIMEOUT_MS`
  // (30s), so link_up_ going false already means a full 30s of silence, not
  // a blip.
  if (!this->link_up_) {
    if (this->log_.error) {
...
bool Runner::refuse_if_set_interlocked_(protocol::KeyMask mask) const {
  ...
  // Loud on purpose: page 27 ("Reset") writes and has never been tried, so a
  // Set reaching the unit while any diagnostic page is showing is exactly the
  // mistake this exists to catch, not routine operation to log quietly.
  if (this->log_.error) {
    this->log_.error(
...
void Runner::recover() {
  // The one place guaranteed to run after ANY root sequence fails, however it
  // failed -- the backstop for "must release the keypad on every exit path".
  // Individual primitives already release whatever they themselves asserted
  // (HoldUntil::on_finish in particular) -- this covers anything they didn't
  // get the chance to, and costs nothing to call redundantly (release() with
  // nothing asserted is a no-op).
  ...
  // adjusts the value under the cursor rather than navigating -- the 14C
  // walked to 19C failure -- and that applies to the FIRST Up just as much as
  // any later one. Only Set is safe inside an editor, and pressing Set here
  // would commit whatever half-finished value the failed sequence left
  // behind, which is worse than doing nothing: the unit closes an abandoned
  // editor itself after ~2 minutes and commits nothing when it does, so a
  // failed write leaves the setting untouched, as a failed write should.
  ...
  // The VERIFIED exit gesture, not "mash Up": a second Up press while an
  // editor is still open silently adjusts the value under the cursor rather
  // than exiting it. Exactly one tap is issued here, then this returns --
  // there is no sequence left on the stack to wait on the unit's own
  // ~2-minute menu timeout closing whatever was open, and nothing here needs
  // to block on that: every navigating sequence (GotoMenu especially) is
  // written to work correctly from an unknown starting screen, so the next
  // request() doesn't need to wait for this unwind to finish first.
...
Poll GotoMenu::poll() {
  switch (this->step_) {
    ...
    // far up to go": Up past index 0 does nothing, so this always lands on 0
    // regardless of where the display started -- deliberately NOT counted
    // relative to an assumed current position. Queued as a batch rather than
    // one-at-a-time: Keypad's own queue enforces key_gap_ms between each tap,
    // so this step is "wait for the queue to drain", not five separate steps.
    case QUEUE_UP:
      for (int i = 0; i < 5; i++) {
    ...
    // state. Every real step above has its own explicit case, so this is only
    // reached by corruption. Previously returned DONE here, reporting the
    // navigation as landed when it never ran to completion; FAILED routes
    // through Runner::recover() instead, which is the pattern every other
    // sequence's own out-of-range default follows too.
    default:
      if (this->log().error) {
...
Poll LeaveMenu::poll() {
  switch (this->step_) {
    ...
    // field under the cursor instead of navigating out -- the same 14C->19C
    // trap Runner::recover() guards against. Skip straight to WAIT_EXIT and
    // let the unit's own ~2-minute timeout close the editor untouched.
    case TAP:
    ...
    // Same defensive default as GotoMenu's: an out-of-range step_ is a bug,
    // not a landing state, so this reports FAILED (not DONE) rather than
    // claiming the menu was successfully left when it never was.
    default:
      if (this->log().error) {
...
Poll OpenEditor::poll() {
  switch (this->step_) {
    ...
    case CHECK:
      ...
      this->attempt_++;
      // Exactly one retry: MAX_ATTEMPTS is 2, so this fires once
      // (attempt_ goes 0 -> 1 -> retry) and fails on the second miss.
      return this->attempt_ >= MAX_ATTEMPTS ? Poll::FAILED : this->goto_step(TAP);

    // Same defensive default as GotoMenu's -- a bug reaching here would
    // otherwise report an editor as confirmed open when it was never
    // actually checked, exactly the dropped-Set failure OpenEditor exists to
    // catch, just reached a different way.
    default:
      if (this->log().error) {
...
Poll AdjustField::poll() {
  switch (this->step_) {
    case CHECK: {
      // The guard, not a timeout, is what bounds this loop. Reaching it means
      // either the field genuinely cannot get to the target (Outdoor Temp's
      // guessed range rejecting a value looks identical to a dropped press
      // from here) or something is very wrong; either way the caller still
      // needs to walk the editor out afterwards, same as every other failure.
      if (this->guard_count_ >= this->guard_limit_) {
        return Poll::FAILED;
      }
      // Re-read the target fresh -- see TargetFn's own comment for why this
      // matters (the clock fields) and why it can never fail for the
      // fixed-target reset() overload. FAILED, not a silent stall, if it is
      // genuinely unavailable: there is nothing to adjust towards.
      int want = 0;
      ...
      // NOT an error if this fails to parse -- see class comment.
      int cur = 0;
    ...
    // Never fires the next tap blind. Loops back to CHECK once line2 has
    // actually changed since the tap above, or after CHANGE_TIMEOUT_MS
    ...
    // Same defensive default as GotoMenu's -- would otherwise report a field
    // as adjusted to its target when it was never actually confirmed.
    default:
      if (this->log().error) {
...
Poll ExitEditChain::poll() {
  switch (this->step_) {
    case CHECK:
      ...
      this->commits_++;
      // The one place in the whole component that commits an edit-in-progress
      // on purpose.
      return this->tap_then_(protocol::key_mask(protocol::Key::SET), SETTLE);

    case SETTLE:
      // Longer than editor_open()'s own settle_ms_ (1200ms default) so the
      // next CHECK's answer actually means something.
    ...
    case WAIT_TIMEOUT:
      ...
      // FAILED, not a silent DONE: cascading through Runner::recover()'s
      // single verified Up tap is safer than letting whatever runs next
      // press Up unconditionally.
      return this->elapsed() >= FALLBACK_TIMEOUT_MS ? Poll::FAILED : Poll::RUNNING;

    // Same defensive default as GotoMenu's -- would otherwise report the edit
    // chain as walked closed when it was never actually confirmed.
    default:
      if (this->log().error) {
...
bool parse_clock_day_field(const std::string &line2, int &out) {
  // clock_rendered() first: calling clock_day() on a mid-blink frame (the day
  // blanked, e.g. "    23:49") would read garbage rather than correctly
  // failing.
