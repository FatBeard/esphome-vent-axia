...
**The generated `mhrv/.esphome/build/vent-axia-mhrv/src/main.cpp` contains `secrets.yaml`'s values in plaintext** — the `web_server` password and the API noise PSK both appear as string literals. It is gitignored (`mhrv/.gitignore`) so it never ships, but note that the stage-8 codegen check below deliberately sends you *into* that file: verifying an entity registered means grepping generated code that sits a few lines from the credentials. Grep for the specific symbol (`set_select`, `set_options`, a named entity) rather than dumping regions of the file, or the secrets end up in the transcript — which is exactly how it happened on 13 Aug 2026, costing a credential rotation.
