...
| 14 | Decode the `*` annunciator at line2 column 15 as `humidity_boost` | **Implemented 17 Aug 2026**, from a user-supplied screenshot of the card's own LCD panel (`lovelace-card/IMG_3778.jpeg`) rather than live observation — see the stage 14 notes below. Live capture (line1's content while the annunciator shows, whether `boosting`/`airflow_mode` move alongside it, blink-vs-steady) is still outstanding. |
...
- **Not yet verified live** -- unlike every prior stage in this table, which came from observing 192.168.1.200 directly, this one is built from a single screenshot and the manual's own words. Deliberately built and tested first (host suite plus both firmware targets), to be captured against the live unit during the next actual humidity boost -- see §9's open item on the `continuous_boost()` interaction this raises, and this stage's own PLAN.md entry (verification step 5, plan file) for the specific questions a live capture needs to settle: what line1 reads while the annunciator shows, whether `boosting`/`airflow_mode` move alongside it, whether it holds steady or blinks on real hardware (retro-justifying or correcting the sticky-Flag choice above), and whether the airflow percentage tracks indoor humidity proportionally.
...
8. **OPEN, deliberately unresolved — `humidity_boost` and `continuous_boost()` may interact, and it is recorded rather than guessed at.** §8 stage 14 decoded line2 column 15's alpha annunciator as `humidity_boost`, but what line1 reads while it is showing is not yet known (the stage started from a screenshot, not a live capture). If a humidity boost ever drives line1 to `Boost Airflow` with no countdown, `airflow_mode` would report `Boost Continuous` wrongly after `CONTINUOUS_CONFIRM_MS` — the same trailing-edge shape §4/stage 9 already accepts for a timed boost's own expiry, just triggered by a different cause. **The screenshot narrows this rather than leaving it neutral, and not in the reassuring direction.** `ms_without_countdown_` (status.cpp) has three reset conditions — `!boosting_.active`, a countdown parsed this frame, or purge — and the captured line2 `31%            *` rules out two of them outright: there is no `m` anywhere on the line for `find_countdown_minutes()` to match, and no purge. So the *only* thing that can prevent a false `Boost Continuous` here is `boosting_` never going active, i.e. line1 never reading `Boost Airflow` during a humidity boost. That single variable is exactly what the screenshot cannot show, because it caught the `Summer Bypass On` half of the alternation; the other half is unobserved. Do not read "the screenshot has no countdown" as evidence the hazard is moot — the absent countdown is what *arms* it. The tempting fix, guarding `continuous_boost()` on `!humidity_boost()`, is deliberately NOT implemented: it would equally suppress a *genuine* commanded continuous boost that happens to coincide with high humidity, and there is no live evidence yet for which case (or both, or neither) actually occurs on this hardware. Two wrong explanations in a row for the SW1/2/3 flags (§9 risk 2) is the reason this is being written down instead of guessed at now — the next live humidity-boost capture (§8 stage 14's outstanding verification) is what settles it, not more reasoning from the armchair.
