...
| 16 | The two-lane split: raw bytes for parsing, UTF-8 for presentation — α reaches Home Assistant | **Implemented and confirmed live 18 Aug 2026**, on the strength of stage 15's measurement; `display_line_2` published `31%            α` during a real humidity boost with the raw lane logging `col 15=0xE0` in the same capture. `sanitize()` is gone; `Display` keeps a raw lane (every decoder's byte offsets, unchanged) and a UTF-8 presentation lane (`to_utf8()`, built on `glyphs::ALPHA = 0xE0`), transcoding only a line that actually changed. `has_sensor_boost_annunciator()` is now an exact byte comparison. Dedup moved to the raw lane, closing the glyph-to-glyph blindness `DISPLAY-REVIEW.md` §5 identified. See the stage 16 notes below. |
...
**Confirmed live 18 Aug 2026, ~12:25 unit time**, on a real humidity boost that started after the flash. The two lanes agree in the same `GET /events` capture, which is the whole design demonstrated end to end:

```
raw frame: line2 non-ASCII: col 15=0xE0  (unchanged -- heartbeat)
display_line_2 = "31%            α"   (then "32%            α" as the rate ticked)
```

The raw lane carries the byte to the decoder -- `humidity_boost` published `ON`, so the exact-byte predicate fires correctly on hardware -- while Home Assistant receives the glyph as α. The pre-boost capture an hour earlier is the other half of the check: with the display all-ASCII, line2 published `30%             `, `Tue 12:10       ` and `   C            ` (the blank-temperature invariant intact) with no `<XX>` escapes anywhere, so the transcode does not over-escape printable bytes.

One methodological note, since this project keeps paying attention to what an absence means: a 70s capture taken minutes earlier showed the published α but *no* raw-lane line at all, which looked like a defect in the heartbeat. It was not. The two heartbeats are independent 60s timers anchored to their own last-log times, so a 70s sample can straddle the line2 beat and catch neither edge of it; a 150s capture showed two of each, exactly as designed. Reading zero lines as "broken" would have been the same mistake stage 15's heartbeat was added to prevent, one level up.

The Lovelace card needs no change to show it: `sentinel-remote-card.js` sets the LCD lines via `el.textContent` (UTF-8-safe, no injection), and the `.lcd` font stack (`VT323`, `Share Tech Mono`, `Consolas`, monospace) falls back per character, so α renders even though `VT323` carries no Greek glyphs. Worth confirming by eye on the card rather than assuming, since the fallback face may differ visibly from the surrounding digits.
