...
| 20 | West/normal link: raw, then 2 west / 1 link / 0 no link | `0410 1          ` | Not understood. The tri-state field is legible and reads `1`, but nothing on this unit is known to depend on it, so there is no entity worth naming yet. |
...
**Pages 9/10 are the one real lead.** Every other row above justifies itself with an all-zero capture or absent hardware; 9/10 captured `1022 0 25 00` — nonzero raw, and a momentary time of 25 — against pages 6/7/8's inert `0000 1 0 000 00 `. The "SW4/SW5 not wired on this unit" note is inherited from `mhrv_orig`'s page map and was never tested with a switch asserted, which is exactly the mistake stage 10 caught twice on pages 6/7/8 (see §8): a flag reading OFF is solid, the story invented to explain it is not. The layout differs too — `closed` sits at pos 5 here, not pos 7 — so these are not a copy of the `PAGE_6_FIELDS` row. They fold into the same test as page 05's open question (§8 stage 10).
