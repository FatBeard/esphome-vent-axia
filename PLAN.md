...
- **Diagnostic page 05 carries the switched live, and nobody had looked at it.** Pages 6/7/8 are byte-identical (`0000 1 0 000 00 `) asserted and released, so there was no column to retarget there. Page 05 was not in the `PAGES` table at all:
  ...
  The decisive comparison is the middle row. **Column 0 stayed `0` right through a genuine commanded boost** (line2 `48%       30m`, `boost_active` ON, `airflow_mode` `Boost 30 min` in the same capture), so it is a switched-live flag and not a boosting flag. Cols 5-6 (`05`) and cols 13-15 (`030`, very likely the overrun period in minutes) move with it, but the two samples a minute apart both read `030`, which cannot separate a configured period from a countdown — left undecoded and recorded as open rather than published under a name that cannot be justified. Note the whole page arrives on the ~15-minute scrape, so it is stale by construction: an explanation in Home Assistant, never a precondition for refusing a command.
